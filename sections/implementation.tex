...
foundational data structure that they are based upon. \\ \\ A world often
referred to as a possible world describes a possible set of information in a
given environment. Most information is unknown to an agent in its environment,
...
combination of agents of roles. The world object itself stores this, is active
and marks. Marks and is active\footnote{Is active refers to active and inactive
	worlds} both being previously described in section \ref{sec:Modelling}.\\ The
...
including the agent, the communication is triggered directly from the agent. \\
...
\captionof{algorithmcaption}{The base algorithms that handels the logic}
Get the highest information gain agent algorithm returns an
agent based on different factors \ref*{alg:GetHighInfoGainP}.
...
Algorithm \ref{alg:GetNonAccused} initializes a list of all other agents for each agent at the
...
\captionof{algorithmcaption}{Returns an agent with the most conflicting role claims}
...
previously shown in algorithm \ref{alg:GetHighInfoGainP}. They chose the world
...
a nightly action is performed can be seen in algorithm \ref{alg:nightPhase}.
\begin{algorithm}[H]
...
previously discussed algorithm \ref{alg:updatePossibleWorlds}. As facts are
