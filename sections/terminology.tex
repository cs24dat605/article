...
\end{flalign}
\\ 
In which $\proposition \in \dynlang$. Note that the expression in (11) refers to a model update. This should be understood as $M'$ is the model in which all worlds where $\varphi$ is false or are not contained, are removed. The members of $M'$ is then given by:
